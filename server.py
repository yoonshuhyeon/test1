...
def get_feedback():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT meal_type, AVG(rating), COUNT(*) FROM meal_feedback WHERE date = ? GROUP BY meal_type"
            if not DATABASE_URL else
            "SELECT meal_type, AVG(rating), COUNT(*) FROM meal_feedback WHERE date = %s GROUP BY meal_type",
            (date,)
        )
        feedback = {row[0]: {"average_rating": round(row[1], 1), "total_reviews": row[2]} for row in cursor.fetchall()}
        cursor.close()
        conn.close()
...
def get_feedback_details():
    try:
        ...
        cursor = conn.cursor()
        query = (
            "SELECT rating, feedback, timestamp FROM meal_feedback WHERE date = ? AND meal_type = ? ORDER BY timestamp DESC"
            if not DATABASE_URL else
            "SELECT rating, feedback, timestamp FROM meal_feedback WHERE date = %s AND meal_type = %s ORDER BY timestamp DESC"
        ) if meal_type else (
            "SELECT meal_type, rating, feedback, timestamp FROM meal_feedback WHERE date = ? ORDER BY timestamp DESC"
            if not DATABASE_URL else
            "SELECT meal_type, rating, feedback, timestamp FROM meal_feedback WHERE date = %s ORDER BY timestamp DESC"
        )
        cursor.execute(query, (date, meal_type) if meal_type else (date,))
        rows = cursor.fetchall()
        feedback_list = []
        for row in rows:
            if meal_type:
                rating, fb, ts = row
                feedback_list.append({'rating': rating, 'feedback': fb, 'timestamp': ts})
            else:
                mt, rating, fb, ts = row
                feedback_list.append({'meal_type': mt, 'rating': rating, 'feedback': fb, 'timestamp': ts})
        cursor.close()
        conn.close()
